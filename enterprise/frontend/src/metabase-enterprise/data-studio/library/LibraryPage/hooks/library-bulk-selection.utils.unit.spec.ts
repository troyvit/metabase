...
describe("library-bulk-selection.utils", () => {
  ...
  describe("selectItemRange", () => {
    it("selects selectable items in range sharing the clicked item's section", () => {
      const visible = [
        sectionRoot("data", []),
        tableItem(1),
        subCollection(5, "data"),
        metricItem(9),
        tableItem(2),
      ];
      // range 1..4 anchored on table:2 (data) → metric excluded, section root excluded
      expect(ids(selectItemRange([], visible, 1, 4))).toEqual([
        "table:1",
        "collection:5",
        "table:2",
      ]);
  ...
  describe("getSectionDirectSelectables", () => {
    it("returns only direct selectable children", () => {
      const root = sectionRoot("data", [
        tableItem(1),
        subCollection(5, "data"),
        emptyState(),
      ]);
      expect(ids(getSectionDirectSelectables(root))).toEqual([
        "table:1",
        "collection:5",
      ]);
    });
