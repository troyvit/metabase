...
const getStackInterval = (
  ...
  return isNormalized
    ? { min: 0, max: NORMALIZED_STACK_MAX_PERCENT }
    : { min: negativeTotal, max: positiveTotal };
};

// Bars and areas fill from zero; any other series (a line) marks only its point.
...
export const useAreAllDataPointsOutOfRange = (
  ...
  const range: Interval = { min: yMin, max: yMax };
  ...
  const isDatumVisible = (datum: Datum) => {
    const stackVisible = chartModel.stackModels.some((stackModel) =>
      isStackVisible(datum, stackModel.seriesKeys, range, isNormalized),
    );
